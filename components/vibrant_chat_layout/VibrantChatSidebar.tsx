...
import { PowerIcon, XMarkIcon } from '../icons';
...
interface VibrantChatSidebarProps {
...
const VibrantChatSidebar: React.FC<VibrantChatSidebarProps> = ({ currentUser, activeScreen, onNavigate, onLogout, isOpen, setIsOpen }) => {
    ...
    return (
        <>
            ...
            <aside 
                className={`fixed lg:absolute lg:inset-y-0 lg:left-0 lg:flex lg:flex-col lg:w-64 inset-y-0 left-0 flex flex-col w-64 z-40 transition-transform duration-300 ease-in-out ${isOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 
                bg-sidebar/80 backdrop-blur-xl border-r border-border-default/50 p-4 text-on-sidebar`}
            >
                ...
                <nav className="flex-1 space-y-1 overflow-y-auto">
                    ...
                    {(finalVisibleToolsNavItems.length > 0 || finalVisibleSettingsNavItems.length > 0) && (
...
export default VibrantChatSidebar;