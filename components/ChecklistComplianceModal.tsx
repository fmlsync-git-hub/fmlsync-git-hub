...
import { CheckCircleIcon, XCircleIcon, DocumentWarningIcon, XMarkIcon } from './icons/index';
...
const Modal: React.FC<{ isOpen: boolean; onClose: () => void; title: string; children: React.ReactNode; }> = ({ isOpen, onClose, title, children }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[100] p-4">
            <div className="bg-surface rounded-lg shadow-xl w-full max-w-6xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
                <div className="flex justify-between items-center border-b border-border-default p-4 flex-shrink-0">
                        ...
                        <div className="overflow-x-auto border border-border-default rounded-lg">
                            <table className="min-w-full divide-y divide-border-default">
                                <thead className="bg-surface-soft">
                                    <tr>
                                        <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-text-secondary uppercase tracking-wider w-1/4">Passenger ({complianceData.length} / {categoryFilteredPassengers.length})</th>
                                        {selectedChecklist.requiredDocuments.map(doc => (
                                            <th key={doc} scope="col" className="px-2 py-3 text-center text-xs font-medium text-text-secondary uppercase tracking-wider">{doc}</th>
                                        ))}
                                    </tr>
                                </thead>
                                <tbody className="bg-surface divide-y divide-border-default">
                                    {complianceData.length > 0 ? complianceData.map(row => (
                                        <tr key={row.passengerName}>
                                            ...
                                            {selectedChecklist.requiredDocuments.map(doc => (
                                                <td key={doc} className="px-2 py-4 whitespace-nowrap text-sm text-center">
