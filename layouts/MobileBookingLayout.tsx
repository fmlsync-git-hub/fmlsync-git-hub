...
import DesktopSidebar from '../components/booking_layout/DesktopSidebar';
...
interface MobileBookingLayoutProps {
  currentUser: User & UserSettings;
  onLogout: () => void;
}

const MobileBookingLayout: React.FC<MobileBookingLayoutProps> = ({ currentUser, onLogout }) => {
  const { companies } = useCompanies();
  const { animationStyle, featureFlags, pageLayout: defaultPageLayout } = useBranding();
  
  const [activeScreen, setActiveScreen] = useState<Screen>(
    currentUser.role === 'designer' ? 'ui_designer' : 'dashboard'
  );
  const [designerPreviewScreen, setDesignerPreviewScreen] = useState<Screen>('ui_designer');
  ...
  const handleNavigation = (screen: Screen) => {
    if (currentUser.role === 'designer') {
      setDesignerPreviewScreen(screen);
    } else {
      setActiveScreen(screen);
      setEditTargetPassenger(null);
    }
  };
  ...
  const getContainerClass = () => {
      switch (effectivePageLayout) {
          case 'wide': return 'max-w-screen-2xl mx-auto shadow-2xl';
          case 'large': return 'max-w-[1400px] mx-auto shadow-2xl';
          case 'boxed': return 'max-w-7xl mx-auto shadow-2xl my-4 rounded-xl overflow-hidden border border-border-default';
          case 'half': return 'max-w-[720px] mx-auto shadow-2xl my-4 rounded-xl overflow-hidden border border-border-default';
          case 'full': 
  ...
  return (
    <div className="min-h-screen w-full bg-background-margin flex flex-col items-center">
        <div className={`h-screen w-full relative bg-background text-on-background flex flex-col ${getContainerClass()}`}>
            <div className="flex flex-1 overflow-hidden relative w-full">
                 <DesktopSidebar
                  currentUser={currentUser}
                  activeScreen={currentUser.role === 'designer' ? designerPreviewScreen : activeScreen}
                  onNavigate={handleNavigation}
                  onLogout={onLogout}
                />
                
                <div className="flex-1 flex flex-col lg:pl-64 min-w-0 w-full h-full">
                  <main className="flex-1 overflow-y-auto py-4 sm:py-6 lg:py-8 pb-24 lg:pb-8 w-full">
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
                      <Suspense fallback={<LoadingSpinner />}>
                        <div key={currentUser.role === 'designer' ? 'ui_designer' : activeScreen} className={`${animationClass} w-full`}>
                        {renderScreen()}
                        </div>
                      </Suspense>
                    </div>
...
export default MobileBookingLayout;
