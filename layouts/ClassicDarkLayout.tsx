...
import ClassicDarkHeader from '../components/classic_dark_layout/ClassicDarkHeader';
import ClassicDarkBottomNav from '../components/classic_dark_layout/ClassicDarkBottomNav';
...
interface ClassicDarkLayoutProps {
  currentUser: User & UserSettings;
  onLogout: () => void;
}

const ClassicDarkLayout: React.FC<ClassicDarkLayoutProps> = ({ currentUser, onLogout }) => {
    ...
    <div className="min-h-screen w-full bg-background-margin flex flex-col items-center">
        <div className={`h-screen w-full flex flex-col bg-background text-on-background ${pageLayout === 'wide' ? 'max-w-screen-2xl mx-auto shadow-2xl' : pageLayout === 'large' ? 'max-w-[1400px] mx-auto shadow-2xl' : pageLayout === 'boxed' ? 'max-w-7xl mx-auto shadow-2xl my-4 rounded-xl overflow-hidden border border-border-default' : pageLayout === 'half' ? 'max-w-[720px] mx-auto shadow-2xl my-4 rounded-xl overflow-hidden border border-border-default' : ''}`}>
            ...
            <button 
                ...
                className="fixed bottom-24 right-6 lg:bottom-6 bg-primary text-on-primary rounded-full p-4 shadow-lg hover:bg-primary-dark transition-transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary z-30"
...
export default ClassicDarkLayout;
