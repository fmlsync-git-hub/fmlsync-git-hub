...
import VibrantChatSidebar from '../components/vibrant_chat_layout/VibrantChatSidebar';
import VibrantChatHeader from '../components/vibrant_chat_layout/VibrantChatHeader';
...
interface VibrantChatLayoutProps {
  currentUser: User & UserSettings;
  onLogout: () => void;
}

const VibrantChatLayout: React.FC<VibrantChatLayoutProps> = ({ currentUser, onLogout }) => {
  ...
  return (
    <div className="min-h-screen w-full bg-background-margin flex flex-col items-center">
        <div className={`h-screen w-full relative z-10 bg-background text-on-background overflow-hidden flex flex-col ${pageLayout === 'wide' ? 'max-w-screen-2xl mx-auto shadow-2xl' : pageLayout === 'large' ? 'max-w-[1400px] mx-auto shadow-2xl' : pageLayout === 'boxed' ? 'max-w-7xl mx-auto shadow-2xl my-4 rounded-xl overflow-hidden border border-border-default' : pageLayout === 'half' ? 'max-w-[720px] mx-auto shadow-2xl my-4 rounded-xl overflow-hidden border border-border-default' : ''}`}>
            <div className="absolute -top-1/4 -left-1/4 w-1/2 h-1/2 bg-primary/20 rounded-full filter blur-3xl opacity-60 z-0 animate-pulse"></div>
            <div className="absolute -bottom-1/4 -right-1/4 w-1/2 h-1/2 bg-purple/20 rounded-full filter blur-3xl opacity-50 z-0 animate-pulse [animation-delay:-2s]"></div>
            ...
            <div className="flex flex-1 overflow-hidden relative w-full">
                ...
                <div className="flex-1 flex flex-col lg:pl-64 relative min-w-0 w-full">
                    <VibrantChatHeader 
            ...
            <button 
                ...
                className="fixed bottom-6 right-6 bg-primary text-on-primary rounded-full p-4 shadow-lg hover:brightness-110 transition-transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary z-30"
...
export default VibrantChatLayout;
