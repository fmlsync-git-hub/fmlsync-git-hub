...
import GlassmorphismHeader from '../components/glassmorphism_layout/GlassmorphismHeader';
import GlassmorphismBottomNav from '../components/glassmorphism_layout/GlassmorphismBottomNav';
...
const GlassmorphismLayout: React.FC<GlassmorphismLayoutProps> = ({ currentUser, onLogout }) => {
  ...
  return (
    <div className="min-h-screen w-full bg-background-margin flex flex-col items-center">
        <div className={`h-screen w-full flex flex-col relative z-10 bg-background text-on-background overflow-hidden ${pageLayout === 'wide' ? 'max-w-screen-2xl mx-auto shadow-2xl' : pageLayout === 'large' ? 'max-w-[1400px] mx-auto shadow-2xl' : pageLayout === 'boxed' ? 'max-w-7xl mx-auto shadow-2xl my-4 rounded-xl overflow-hidden border border-border-default' : pageLayout === 'half' ? 'max-w-[720px] mx-auto shadow-2xl my-4 rounded-xl overflow-hidden border border-border-default' : ''}`}>
            <div className="absolute top-[5%] left-[5%] w-1/3 h-1/3 bg-primary/10 rounded-full filter blur-3xl opacity-70 z-0 animate-pulse"></div>
            <div className="absolute bottom-[5%] right-[5%] w-1/3 h-1/3 bg-purple/10 rounded-full filter blur-3xl opacity-70 z-0 animate-pulse [animation-delay:-2s]"></div>
            ...
            <main className="flex-1 overflow-y-auto py-4 sm:py-6 lg:py-8 pb-24 lg:pb-8 w-full min-w-0 relative z-10">
