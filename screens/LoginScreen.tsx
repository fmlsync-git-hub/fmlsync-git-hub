...
const LoginScreen: React.FC<LoginScreenProps> = ({ onLogin }) => {
  ...
  return (
    <div className="flex items-center justify-center min-h-screen w-full p-4 relative overflow-hidden" style={containerStyle}>
      ...
      <div className="w-full max-w-[380px] p-6 sm:p-8 space-y-6 backdrop-blur-xl rounded-2xl shadow-2xl border relative z-10 animate-slide-up" style={cardStyle}>
        <div className="text-center space-y-2">
            <div className="shadow-lg rounded-2xl p-2.5 inline-block mb-1" style={{ backgroundColor: loginSettings.inputBackgroundColor }}>
                {appLogo ? (
            ...
            <h1 className="text-2xl font-extrabold tracking-tight" style={textStyle}>{appName}</h1>
            <p className="text-sm font-medium opacity-80" style={textSecStyle}>Welcome back! Please enter your details.</p>
        </div>

        <form className="mt-4 space-y-4" onSubmit={handleLogin}>
          <div className="space-y-1.5">
            <label className="block text-xs font-semibold ml-1 uppercase tracking-wider" style={textSecStyle}>Username</label>
            <input
              style={inputStyle}
              className="w-full text-base px-4 py-2.5 border rounded-xl focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-transparent focus:ring-opacity-50 transition-all duration-200 placeholder:opacity-50 backdrop-blur-sm"
              type="text"
          ...
          <div className="space-y-1.5 relative">
            <label className="block text-xs font-semibold ml-1 uppercase tracking-wider" style={textSecStyle}>Password</label>
           ...
           {error && (
                <div className="bg-red-500/10 text-red-500 text-xs p-3 rounded-xl text-center font-medium animate-pulse border border-red-500/20">
                    {error}
                </div>
            )}
          ...
          <div className="pt-2 space-y-4">
            <button
              ...
              style={buttonStyle}
              className="w-full flex justify-center py-3.5 px-6 border border-transparent rounded-xl text-sm font-bold focus:outline-none focus:ring-2 focus:ring-offset-2 transition-all duration-200 shadow-xl transform active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed hover:brightness-110"
            >
              {isLoggingIn ? (
