...
import { updateUserCredentialsCF } from '../services/firebase';
...
const Input: React.FC<{ label: string; id: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void; type?: string; disabled?: boolean; }> = ({ label, id, value, onChange, type = 'text', disabled = false }) => (
    <div>
        ...
        <input id={id} type={type} value={value} onChange={onChange} disabled={disabled} className="w-full px-3 py-2 border border-border-default bg-background rounded-md focus:outline-none focus:ring-1 focus:ring-primary disabled:bg-surface-soft disabled:opacity-70" />
    </div>
);

interface AdminEditOfficerCredentialsModalProps {
    user: User;
    onClose: () => void;
    onSave: () => void;
}

export const AdminEditOfficerCredentialsModal: React.FC<AdminEditOfficerCredentialsModalProps> = ({ user, onClose, onSave }) => {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        setIsSaving(true);
        try {
            // FIX: Pass password OR undefined, not an empty string, which the backend would reject.
            await updateUserCredentialsCF(user.username, user.username, password || undefined);
            onSave();
            onClose();
        } catch (err) {
        ...
        <Modal isOpen={true} onClose={onClose} title={`Edit Credentials for ${user.username}`}>
            <form onSubmit={handleSave}>
                <div className="p-6 space-y-4">
                    ...
                    <Input id="username" label="Username" value={user.username} onChange={() => {}} disabled={true} />
